...
void storeDateTime(DateTime now) {
    // Armazena ano, mês, dia, hora, minuto e segundo na EEPROM
    int address = 0; // Endereço inicial na EEPROM
    EEPROM.put(address, now.year());
    EEPROM.put(address + 2, now.month());
    EEPROM.put(address + 3, now.day());
    EEPROM.put(address + 4, now.hour());
